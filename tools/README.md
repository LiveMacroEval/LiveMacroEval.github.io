...
longer runs to its last target month and the model that replaced it (the model
only, not the configuration); the note under its name -- on the all-quarters
board only -- reads "retired Aug 2026 and replaced by Sonnet 5", or "retired Jul
2026" for an arm without a successor. A quarter tab
...
"w. tool" / "w. multi-agent", which links the plug-ins and the paper's Tool and
Agent Design section (arXiv HTML anchor `#S4.SS6`). A model switch makes a new arm: the private pipeline
